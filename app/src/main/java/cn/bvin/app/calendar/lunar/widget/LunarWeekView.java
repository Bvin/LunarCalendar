...
public class LunarWeekView extends WeekView {
    ...
    public LunarWeekView(Context context) {
        super(context);
    ...
    protected void onDrawText(Canvas canvas, Calendar calendar, int x, boolean hasScheme, boolean isSelected) {
        ...
        float y1 = mTextBaseLine + top;
        float y2 = mTextBaseLine + mItemHeight / 10;
        float tmp = y2;
        y2 = y1;
        y1 = tmp;

        if (isSelected) {
            canvas.drawText(String.valueOf(calendar.getDay()), cx, y1,
                    mSelectTextPaint);
            canvas.drawText(calendar.getLunar(), cx, y2, mSelectedLunarTextPaint);
        } else if (hasScheme) {
            canvas.drawText(String.valueOf(calendar.getDay()), cx, y1,
                    calendar.isCurrentDay() ? mCurDayTextPaint :
                            calendar.isCurrentMonth() ? mSchemeTextPaint : mOtherMonthTextPaint);

            canvas.drawText(calendar.getLunar(), cx, y2,
